...
Increasing dimension adds an additional othogonal axis. 

<p>If you want more high-dimensional shapes there is an R package, `geozoo`, which will generate cubes, spheres, simplices, torii, klein bottles, various polytopes, ... 
...
p <- ggplot(data = df, aes(x = x, y = y) ) +
       ...
       theme_void() +
       coord_fixed()
pg <- ggplotly(p) %>% animation_opts(200, redraw = FALSE, 
                                     easing = "linear")
...
--
<p>
Set A are genes identified by Sarah Romanes [multiDA](https://github.com/sarahromanes/multiDA) procedure; set B are a random sample of genes.
--
<p>
